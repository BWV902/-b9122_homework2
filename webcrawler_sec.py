...
# (for example: the latest release https://www.sec.gov/news/press-release/2022-186)
# and rewrite the seed_url to match the format for later comparison:
hseed_url = "https://www.sec.gov/news/press-release"


urls = [seed_url] 
seen = [seed_url]
opened = []
targetUrls = {} # stores each url and its text
outNumUrl = 20 # exactly output first 20 links
...
while len(urls) > 0 and (len(targetUrls) < outNumUrl):
    ...
    if ("charges" in text) and (curr_url not in targetUrls) and (curr_url != seed_url):
        content = soup.get_text().replace('\n', ' ')
        targetUrls[str(curr_url)] = content
...
print("num. of webpages contain \'charges'\: %d" %len(targetUrls))
